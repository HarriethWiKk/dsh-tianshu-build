...
### Key design language extracted from reference projects (recon summary, with evidence)
...
## 1. Concept A "Chart" — grok lineage, information-density first

**一句话**：把 dsh TUI 从"对话滚动"升级为"工作台视图"——欢迎页是功能入口，主界面按信息优先级堆叠，一切状态可见。

### 1.1 Welcome page (first screen)

```
┌─ dsh 航图 ───────────────────────────────────────────── 2026-08-12 ─┐
│                                                                      │
│        ████████                                                        │
│        ██ dsh ██      DeepSeek Harness  v0.x                          │
...
### 1.3 Mapping to the existing architecture
...
## 2. Concept B "Dive" — Claude lineage, conversational immersion

**一句话**：极简对话流 + 一个强大的多行输入框——输入框是全部 UI 的核心，信息都在输入框的 footer 里。

### 2.1 First-screen welcome

```
   ██████████████████
   ██   d s h      ██     DeepSeek Harness v0.x
   ██  ██████████  ██
   ██████████████████     ·····································
                          首次使用？三步就绪：
...
### 2.3 Mapping to the existing architecture
...
## 3. Concept C "Workstation" — fusion, dsh-flavored session workbench

**一句话**：grok 的入口效率 + claude 的输入框质感 + dsh 独有的多会话/btw/审批/面板——面向重度多会话用户。

### 3.1 Welcome page (session-restore first)

```
┌─ dsh ────────────────────────────────────────────────────────────┐
│                                                                   │
│   ██ dsh ██   DeepSeek Harness v0.x · graphite                    │
│                                                                   │
│   ╭─ 恢复会话 ────────────────────────────────╮                    │
...
### 3.3 Mapping to the existing architecture
...
## 5. Counter-evidence / reproduction (assertions to verify before design freeze)
