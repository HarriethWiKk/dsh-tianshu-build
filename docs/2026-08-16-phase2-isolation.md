...
1. **默认 home 独立化**:`dsh-paths` `DSH_HOME_DIR_NAME` → `.dsh-tianshu`;
   paths.spec 6/6、workspace-context 143/143、settings-local 全绿。
   排查中修复两坑:① paths `src/` 残留旧编译产物(index.js 被 vite 优先
   解析,导致测试读旧值)——删除残留 + tsconfig.base.json 补
   `@huiliyi37/dsh-paths → src` 映射(项目惯例,统一模块实例);
   ② workspace-context 依赖 node:os mock 的用例在双实例下脆弱——改显式
   dshHome(默认 home 解析由 paths.spec 覆盖)。
2. **migrate-home**:新子命令(旧 `~/.dsh` → 新默认 home 整体复制,不删旧,
   幂等);尊重 `$DSH_HOME`(实测被 env 覆盖时目标跟随)。
3. **命名统一**:命令 `tianshu` → `oh-my-tianshu`、npm 包
   `@huiliyi37/dsh-tianshu` → `@huiliyi37/oh-my-tianshu`;全仓包名引用 +
   文档命令用法替换(84 文件);品牌/产品标识(Tianshu/天枢/tianshu-harness)
   与插件名 dsh-tianshu-tui 保留;历史基线快照(.artifacts)不动。
4. **本机落地**:`~/.dsh` 数据合并进 `~/.dsh-tianshu`(只增不删);官方
   `tui` profile 恢复(bundles = @deepseek-ai/dsh-base + dsh-tianshu-tui)。

### 遗留(发布级,待确认)

- 推送 github(3 个提交:1c738040 home 隔离 / 92425431 改名)
- npm 发布新包名 `@huiliyi37/oh-my-tianshu` + deprecate 旧包
  `@huiliyi37/dsh-tianshu`
- 全局命令重装(`npm i -g @huiliyi37/oh-my-tianshu`),旧 `tianshu` 命令
  由用户自行卸载
